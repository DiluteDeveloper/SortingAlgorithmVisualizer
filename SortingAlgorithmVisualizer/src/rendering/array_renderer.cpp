...
void ArrayRenderer::updateMesh() {
	...
	float horiScale = ((rOptions.maxRight - rOptions.maxLeft) / (float)arrayObj.size()); // Size of each element+distancetonextelement to be able to fit it all on screen comfortably
	float vertScale = ((rOptions.maxHeight - rOptions.bottomHeight) / heightComplexity); // 100 would be the size of the largest element hypothetically.
	...
	for (size_t i = 0; i < arrayObj.size(); i++)
	{
		...
		vertices.push_back({ rOptions.maxLeft + blockX, rOptions.bottomHeight }); // bottom left
		vertices.push_back({ rOptions.maxLeft + marginX, rOptions.bottomHeight }); // bottom right
		vertices.push_back({ rOptions.maxLeft + marginX, rOptions.bottomHeight + (vertScale * arrayObj[i])}); // top right
		vertices.push_back({ rOptions.maxLeft + blockX, rOptions.bottomHeight + (vertScale * arrayObj[i]) }); // top left
...
void ArrayRenderer::swap(uint16_t a, uint16_t b) {
